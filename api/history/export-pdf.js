...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (let i = 0; i < logs.length; i++) {
      ...
      doc.text(String(i + 1), col1, y, { lineBreak: false });
      doc.text(s.seedTag || "-", col2, y, { width: 140, lineBreak: false });
      doc.text(formatProcessLabel(s.process), col3, y, {
        width: 70,
        lineBreak: false,
      });
      doc.text(String(s.block ?? "-"), col4, y, { lineBreak: false });
      doc.text(String(s.lot ?? "-"), col5, y, { lineBreak: false });
      doc.text(String(s.quantity ?? "-"), col6, y, { lineBreak: false });
